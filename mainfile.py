...
cap = cv2.VideoCapture("http://192.168.29.28:8080/video")
...
def center_text(image, text):
    ...
    text_x = (image.shape[0] - text_size[0]) // 2
    text_y = (image.shape[1] - text_size[1]) // 2
    cv2.putText(
        image, text, (text_x, text_y), font, 2, (255, 0, 255), 5, cv2.LINE_AA
    )
...
while True:
    ...
    if pressed_key == 27:
        ...
        cv2.imwrite("output/scanned_real" + str(count) + ".jpg", warped)
        count +=1
